...
import { Search, BookOpen, Gauge, Zap, Battery, Disc3, Car, Settings, Fan, Wrench, ExternalLink, Menu, X } from 'lucide-react';
import './styles.css';
...
function App() {
  ...
  return <div className={workshop?'app workshop':'app'}>
    <header>
      ...
      <button className="openPdf" onClick={()=>openPdf(1)}>Open PDF</button>
    </header>
    <div className="layout">
      <aside className={drawer?'sidebar open':'sidebar'}>
        ...
        <nav>{filtered.map(s=>{const I=s.icon; return <button key={s.id} className={s.id===section.id?'selected':''} onClick={()=>{setActive(s.id);setDrawer(false)}}><I size={17}/><span>{s.title}</span><small>p{s.pages.join(',')}</small></button>})}</nav>
      </aside>
      <main>
        <section className="card hero">
          <div className="pageBadge"><Icon size={16}/> Pages {section.pages.join(', ')}</div>
          <h2>{section.title}</h2><p>{section.summary}</p>
          <div className="buttons">{section.pages.map(p=><button key={p} onClick={()=>openPdf(p)}>View scan p{p} <ExternalLink size={14}/></button>)}</div>
        ...
        <section className="card">
          ...
          <div className="gallery">{diagrams.map(([name,page])=><button key={name} onClick={()=>openPdf(page)}><BookOpen size={30}/><strong>{name}</strong><span>Open scanned page {page}</span></button>)}</div>
        </section>
      </main>
    </div>
  </div>;
